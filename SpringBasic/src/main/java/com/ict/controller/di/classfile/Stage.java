...
public class Stage {
	...
	// 5버전 이하 버전 사용시 결합이 자동으로 되지 않는다.  
	// @Autowired를 입력시 해당 자료형과 일치하는 부품이 공장 내에 존재하면 자동으로 결합해줍니다.
	// 변수 위, 생성자 위 중 하나를 고르시면 됩니다.
	// 5버전은 멤버변수가 1개가 있는경우 @Autowired없어도 자동으로 주입이 된다. 
	//@Autowired //root-context에서 직접 자료를 입력할 경우 필요없음.
	//@Qualifier("popSinger") // balladsinger에 우선권을 준다. 
	private Singer singer; // 가수 멤범변수
	
	
	// 4버전까지 @Autowired 사용시 아무 작업도 하지 않는 디폴트 생성자가 추가되어야 합니다.
	// 5버전에도 @Qualifier 어노테이션 사용시는 디폴트 생성자가 필요합니다.
	public Stage() {}
