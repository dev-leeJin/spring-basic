...
public class BoardController {
	...
	// /boardList주소를 get방식으로 선언해주세요.
	// 메서드 내부에서는 boardMapper의 .getAllList를 호출해 그 결과를 바인딩합니다.
	@GetMapping("/boardList")
	// @RequestParam(name="사용할변수명", defaultValue="지정하고싶은기본값") 을 변수 왼쪽에 붙이면 시작페이지를 1페이지로 설정
	// @PathVariable을 사용할 경우 defaultValue를 직접 줄 수 없으나, required=false를 이용해 필수입력을 안받게 처리한 후
	...
	// @PathValiable적용 방식으로 바꿔보세요. , boardList에서 제목클릭시 넘어오게해주세요.
	// /boardDetail뒤에 /{bno}를 적고 변수 앞에 @PathVariable을 붙인다. 
	@GetMapping("/boardDetail/{bno}") 
	public String boardDetail(@PathVariable long bno, Model moder) {
		BoardVO board = service.select(bno);
		moder.addAttribute("board",board);
		return "boardDetail";
	}
	...
	// /boardUpdateForm를 Post방식으로 접속하는 form 연결 메서드를 만들겠습니다.
	...
	// update(BoardVO)를 실행해서ㅡ 폼에서 날려준 데이터를 토대로
	// 해당 글의 내용이 수정되도록 만들어주시면 됩니다.
	// 수정 후에는 수정요청이 들어온 글 번호의 디테일페이지로 리다이렉트 시켜주세요.
	@PostMapping("/boardUpdate")			// keyword, searchType, pageNum을 받기 위해 선언
